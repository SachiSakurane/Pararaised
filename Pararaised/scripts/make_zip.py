...
def  main():
  ...
  installer = "\installer\Pararaised Installer.exe"
   
  if demo:
    installer = "\installer\Pararaised Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Pararaised manual.pdf" 
  ]
  ...
  ZIPNAME = "Pararaised-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Pararaised-v" + FULLVERSIONSTR + "-win-demo.zip"
